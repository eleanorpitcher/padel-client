...
function UserProfile() {
  ...
  function getUser() {
    axios
      ...
      .then((response) => {
        ...
        updateRank(oneUser);
        const gamesBeforeToday = oneUser.gamesPlayed.filter(
  ...
  useEffect(() => {
    if (user) {
      ...
      const filteredEvents = user.gamesPlayed.filter(
        ...
        <div className="flex flex-col  p-20 pb-4   w-4/5 mx-auto h-screen ">
          <div className=" flex items-center">
            ...
            <div className="flex justify-end gap-2 ml-16 text-4xl gap-x-16  mb-4 p-4  rounded-md shadow-md bg-[#ffffff] bg-opacity-70 ">
              ...
              <div className="flex flex-col items-center justify-end">
                ...
                <p
                  className={`text-green2_color mt-6 font-bold ${
                    rank === "Not enough games" ? "text-2xl " : " text-5xl"
                  }`}
                >
                  {rank}
                </p>
          ...
          <div className="flex flex-col mx-auto mt-12 rounded-md shadow-md bg-white pr-2 pl-2 bg-opacity-100 w-2/3">
            <div className="flex justify-between w-full">
              ...
              {user && loggedInUser && user._id === loggedInUser._id && (
                <img
                  ...
                  className="w-7 h-7 transition duration-500 transform hover:scale-125 mt-4 mr-7"
                  src={editIcon}
                  alt="Edit"
                />
              )}
            ...
            {isEditing && (
              <img
                ...
                src={saveIcon}
                className="w-7 h-7  self-end transition duration-500 transform hover:scale-125 mb-4 mr-4 mt-2"
              />
            )}
          </div>
          ...
          <div className="mt-20 flex gap-8 mx-auto rounded-md shadow-md mb-16 pl-12 pb-12 pr-12 pt-6 bg-white bg-opacity-100   ">
            <div className="flex flex-col items-center">
              <h1 className="text-4xl font-bold text-green2_color mb-4">
                Past Events
              </h1>
              {pastEvents.length > 0 ? (
                pastEvents.map((event) => {
                  const userResult = event.results.find(
                    (result) => result.player.toString() === user._id.toString()
                  );
                    ...
                    <Link key={event._id} to={`/events/${event._id}`}>
                      <div className="w-[500px] h-96 mb-8 rounded-2xl overflow-hidden relative transition duration-500 hover:scale-105">
                        <img
                          ...
                          src={event.photo}
                          alt={`Event photo for ${event.name}`}
                        />
            ...
            <div className="flex flex-col items-center">
              ...
              {user.gamesPlayed.some(
                (oneEvent) => new Date(oneEvent.date).getTime() > Date.now()
              ) ? (
                user.gamesPlayed
                  .filter(
                    (oneEvent) => new Date(oneEvent.date).getTime() > Date.now()
                  ...
                  .map((oneEvent) => {
                    const userResult = oneEvent.results.find(
                      (result) =>
                        result.player.toString() === user._id.toString()
                    );
                        ...
                        <div  className="w-[500px] h-96 mb-8 rounded-2xl overflow-hidden relative transition duration-500 hover:scale-105">
                          <div className="block w-full h-full">
                            <img
                              className="w-full h-full object-cover opacity-20"
                              src={oneEvent.photo}
                              alt={`Event photo for ${oneEvent.name}`}
                            />
                            <div className="absolute gap-1 inset-0 flex flex-col text-center items-center justify-center text-black text-xl font-bold z-10">
                              <Link to={`/events/${oneEvent._id}`}>
                                <p className="text-4xl">{oneEvent.name}</p>
                              </Link>
                              <p className="opacity-100 text-green">
                              ...
                              <div className="absolute bottom-0 w-full text-center mb-4">
                                <button
                                  ...
                                  onClick={() =>
                                    deleteParticipation(oneEvent._id)
                                  }
                                >
